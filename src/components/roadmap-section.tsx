...
export default async function RoadmapSection() {
  ...
  const roadmapFaqs = faqs.filter(faq => faq.category === "roadmap");

  const faqsWithContent = await Promise.all(
    roadmapFaqs.map(async (faq) => {
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-40">
      <h2 className="text-3xl font-bold mb-8 text-center">Unsere Roadmap</h2>
